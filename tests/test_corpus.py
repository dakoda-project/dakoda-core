...
def test_document(test_cas, test_corpus):
    ...
    assert doc.learner.sentences[0].span == (0, 84)
    assert len(doc.learner.sentences[0].text) == 84

    pos = doc.learner.pos_tags[0]
    pos_th = doc.target_hypothesis.pos_tags[0]

    assert pos.span == (0, 2) and pos.value == "NE"
    assert pos_th.span == (0, 2) and pos_th.value == "NE"
    assert pos_th == pos
