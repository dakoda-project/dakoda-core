...
from dakoda.query import (
    field, value, view, annotation, count, mean_filter, eq, AndPredicate,
    neq, lt, le, gt, ge, contains, startswith, endswith, in_list, not_in_list,
    is_null, is_not_null, custom, sum_filter, min_filter, max_filter,
    std_filter, var_filter, TruePredicate, FalsePredicate, NotPredicate,
    OrPredicate, ColumnPredicate
)
import polars as pl
...
def test_empty_composite_predicates(sample_index):
    ...
    assert empty_and.evaluate(sample_index).sum() == len(sample_index)  # empty AND = True
